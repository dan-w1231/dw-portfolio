...
const NVImages = [
  ...
  },
  {
    image: function Approved() {
      const isMobile = useIsMobile();
      return (
        <motion.div key="Approved" className="relative w-full md:min-h-[540px] lg:min-h-[auto] flex items-center justify-center bg-ice-800/30 dark:bg-ice-900/5 transition-bg duration-900 rounded-4xl md:rounded-5xl xl:rounded-6xl shadow-lg min-w-[220px] md:min-w-0">
          <motion.div className="w-auto lg:w-[73%] h-full" initial={{ opacity: 0, y: 32 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.25 }} transition={{ duration: 1.2 }}>
            <Zoomy initialScale={1.2}>
...
export default function NewVehicle() {
  ...
  const y = useTransform(scrollYProgress, [0, 1], initialWindowWidth < 768 ? ["0%", "0%"] : ["0px", "600px"]);
